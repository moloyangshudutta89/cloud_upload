...
class Upload:
    ...
    ###initializing the credentials dictionary###
    ...
    ###scanning for files from the given directory###
    def read_from_dir(self):

        if self.directory:
            print(f"Reading files from directory: {self.directory}")
            """get all segregated files from the directory and its sub directory"""
            ...
            if len(files) > 0:
                ...
                ####Calling respective bucket upload functions######
